...
#include "tests/tt_metal/test_utils/df/df.hpp"
...
#include "tests/tt_metal/test_utils/deprecated/tensor.hpp"
#include "tests/tt_metal/test_utils/tilization.hpp"
#include "tests/tt_metal/tt_metal/common/matmul_test_utils.hpp"
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        try {
            ...
            std::tie(M, input_args) = test_args::get_command_option_uint32_and_remaining_args(input_args, "--m", 4072);
            std::tie(N, input_args) = test_args::get_command_option_uint32_and_remaining_args(input_args, "--n", 4072);
            std::tie(K, input_args) = test_args::get_command_option_uint32_and_remaining_args(input_args, "--k", 4072);
        ...
        for (uint32_t i = 0; i < num_tests; ++i) {
            if (fast_dispatch_mode == false) {
                ...
                log_info(
                    LogTest,
                    "time duration: {:.15}us ({}cycles) rmax_tflops {:.2f}",
                    execution_time,
